...
if __name__ == '__main__':
    ...
    acs_settings = AntColony.Settings(alpha=5.0, beta=5.0)
    eas_settings = AntColony.Settings(elitist=3.0)
    mmas_settings = AntColony.Settings(max_limit=45.0, min_limit=10.0)
    ras_settings = AntColony.Settings(rank_cutoff=5)
 
    as_colony = AntColony(AntColony.Variation.ANT_SYSTEM, as_settings)
    acs_colony = AntColony(AntColony.Variation.ANT_COLONY_SYSTEM, acs_settings)
    ...
    path, dist = acs_colony.solve(initial_state, tsp.successors, tsp.goal)
    print("Ant Colony System: ", [p.current_node for p in path], dist)
